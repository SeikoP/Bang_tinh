...
class SecurityAnalyzer(CodeAnalyzer):
    ...
    def __init__(self, project_root: Path):
        ...
        self.credential_patterns = [
            (
                re.compile(
                    r'password\s*=\s*["\'](?!.*\{.*\})([^"\']{3,})["\']', re.IGNORECASE
                ),
                "password",
            ),
            (
            ...
            (
                re.compile(
                    r'secret\s*=\s*["\'](?!.*\{.*\})([^"\']{10,})["\']', re.IGNORECASE
                ),
                "secret",
            ),
            (
                re.compile(
                    r'token\s*=\s*["\'](?!.*\{.*\})([^"\']{10,})["\']', re.IGNORECASE
                ),
                "token",
            ),
            (
                        ...
                        for match in matches:
                            ...
                            if self._is_likely_credential(value):
                                finding = Finding(
                                    ...
                                    details={"type": cred_type, "line": line.strip()},
                                )
        ...
        for node in ast.walk(func_node):
            ...
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name):
                    if any(
                        indicator in node.func.id.lower()
                        for indicator in validation_indicators
                    ):
                        return True
                elif isinstance(node, ast.Attribute):
                    if any(
                        indicator in node.func.attr.lower()
                        for indicator in validation_indicators
                    ):
                        return True
