...
def python_file_structure(draw):
    ...
    for i in range(num_files):
        ...
        for d in range(depth):
            dir_name = draw(st.sampled_from(['ui', 'services', 'database', 'core', 'utils', 'tests']))
...
def sql_injection_code(draw):
    ...
    injection_type = draw(st.sampled_from([
        'string_concat',
        'format_string',
        'f_string',
        'percent_format'
...
def hardcoded_credential_code(draw):
    ...
    cred_type = draw(st.sampled_from([
        'password',
        'api_key',
        'secret',
        'token'
    ...
    value = draw(st.text(min_size=10, max_size=30, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))))
...
def test_property_1_complete_file_discovery(file_structure):
    ...
    **Validates: Requirements 1.1**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert created_set.issubset(discovered_set), \
...
def test_property_2_architectural_violation_detection(violation_code):
    ...
    **Validates: Requirements 1.2**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        total_violations = (results['layer_violations'] + 
                          results['tight_coupling_issues'])
        
        assert total_violations > 0, \
            "Analyzer should detect architectural violations"
...
def test_property_3_code_smell_detection(code):
    ...
    **Validates: Requirements 1.3**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        total_smells = (results['complexity_issues'] + 
                       results['long_methods'] + 
                       results['long_parameter_lists'])
        
        assert total_smells > 0, \
            "Analyzer should detect code quality issues"
...
def test_property_4_security_vulnerability_detection(vuln_code):
    ...
    **Validates: Requirements 1.4**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        total_issues = (results['credential_issues'] + 
                       results['sql_injection_risks'])
        
        assert total_issues > 0, \
            "Analyzer should detect security vulnerabilities"
...
def test_property_5_risk_classification_consistency(category, description, file_path):
    ...
    **Validates: Requirements 1.6**
    ...
    for risk_level in [RiskLevel.LOW, RiskLevel.MEDIUM, RiskLevel.HIGH, RiskLevel.CRITICAL]:
        ...
        assert finding.risk_level in [RiskLevel.LOW, RiskLevel.MEDIUM, 
        ...
        assert finding.risk_level == finding2.risk_level, \
