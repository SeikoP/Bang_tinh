...
def invalid_config_dict(draw):
    ...
    if invalid_type == 'invalid_port':
        ...
        base_config['LOG_LEVEL'] = draw(st.text(min_size=1, max_size=20).filter(
                    ...
                    for stmt in ast.walk(item):
                        if isinstance(stmt, ast.Call):
                            if isinstance(stmt.func, ast.Name):
                                ...
                                if stmt.func.id[0].isupper():  # Class names start with uppercase
                                    has_internal_instantiation = True
                    
                    # Property: If has DI, should have parameters and no internal instantiation
                    if has_di:
                        assert param_count > 0, "DI pattern should have constructor parameters"
    ...
    **Validates: Requirements 2.7**
    ...
    with patch.dict(os.environ, config_dict, clear=True):
        try:
            ...
            is_valid = (
                1024 <= port <= 65535 and
                log_level in ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'] and
                width >= 800
            )
