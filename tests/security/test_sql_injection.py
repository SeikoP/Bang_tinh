...
class TestSQLInjection:
    ...
    def test_bank_repository_sql_injection_in_content(self):
        ...
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='bank_history'")
            assert cursor.fetchone() is not None, "Table was dropped - SQL injection successful!"
