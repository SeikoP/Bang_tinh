...
class TTSService(QObject):
            ...
            if CACHE_DIR.exists():
                for f in CACHE_DIR.glob("*.mp3"):
                    if f.stat().st_size > 500:
                        ...
                        try:
                            os.unlink(str(f))  # Delete 0-byte or corrupt files
                        except:
                            pass
        except Exception as e:
    ...
    def speak_transaction(self, amount: str, sender_name: Optional[str] = None):
        ...
        try:
            ...
            if amount_int == 0 and "000,000" in amount and len(amount_clean) >= 6:
                ...
                amount_int = 1000000
                self.logger.warning(
                    f"TTS: Interpreting {amount} as 1,000,000 based on user request"
                )
            ...
            if cache_key in self._cache:
                self.logger.info(
                    f"TTS Latency: ZERO (Cache hit for {action_key} {amount_int})"
                )
                self._play_file(self._cache[cache_key], message, cache_key)
                return

            self.logger.info(
                f"TTS: Generating for {action_key} {amount_int} -> '{message}'"
            )
    ...
    def _play_file(
        ...
        try:
            ...
            if not p.exists() or p.stat().st_size < 500:
                self.logger.warning(
                    f"Cached file invalid: {file_path}. Retrying generation."
                )
                if p.exists():
                    try:
                        os.unlink(file_path)
    ...
    def _pregenerate_common_amounts(self):
        ...
        self.logger.info(
            f"TTS: Batch pre-generating {len(items_to_generate)} amounts..."
        )
        ...
        worker.all_done.connect(
            lambda: self.logger.info("TTS: Batch pre-generation completed.")
        )
        worker.error.connect(
            lambda e: self.logger.error(f"TTS batch pre-gen error: {e}")
        )
        worker.finished.connect(lambda: self._cleanup_worker(worker))
        self._workers.append(worker)
        worker.start()
