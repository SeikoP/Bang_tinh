...
class ApplicationProfiler:
    ...
    def log_system_metrics(self):
        ...
        self.logger.info(f"  Memory: {metrics.get('memory_percent', 0):.1f}% ({metrics.get('memory_used', 0) / 1024 / 1024 / 1024:.2f} GB)")
        self.logger.info(f"  Disk: {metrics.get('disk_percent', 0):.1f}% ({metrics.get('disk_free', 0) / 1024 / 1024 / 1024:.2f} GB free)")
        self.logger.info(f"  Process Memory: {metrics.get('process_memory', 0) / 1024 / 1024:.2f} MB")
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> str:
        ...
        if metrics:
            ...
            lines.append(f"  Process Memory: {metrics.get('process_memory', 0) / 1024 / 1024:.2f} MB")
            lines.append("")
        ...
        if self.metrics:
            ...
            # Sort by duration
            sorted_metrics = sorted(self.metrics, key=lambda m: m.duration or 0, reverse=True)
            ...
            for metric in sorted_metrics:
                ...
                if metric.memory_delta is not None:
                    lines.append(f"    Memory: {metric.memory_delta / 1024 / 1024:.2f} MB")
