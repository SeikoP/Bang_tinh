...
class ProductRepository(IProductRepository):
    ...
    def add(name: str, large_unit: str, conversion: int, unit_price: float) -> int:
        ...
        try:
            ...
            with get_connection() as conn:
                ...
                cursor.execute(
                    "INSERT INTO session_data (product_id) VALUES (?)", (product_id,)
                )
    ...
    @staticmethod
    def update(
        product_id: int, name: str, large_unit: str, conversion: int, unit_price: float
    ) -> bool:
        """Cập nhật sản phẩm"""
        try:
            # Validate inputs
        ...
        try:
            with get_connection() as conn:
                ...
                if soft_delete:
                    ...
                    cursor.execute(
                        "DELETE FROM session_data WHERE product_id=?", (product_id,)
                    )
    ...
    def toggle_favorite(product_id: int) -> bool:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to toggle favorite: {str(e)}", "toggle_favorite"
            )
        ...
        try:
            ...
            if handover < 0:
                raise ValidationError(
                    "Handover quantity cannot be negative", "handover"
                )
            if closing < 0:
    ...
    def reset_all() -> bool:
        ...
        try:
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE session_data SET handover_qty = 0, closing_qty = 0"
                )
                return True
        except Exception as e:
...
class HistoryRepository(IHistoryRepository):
    ...
    def save_current_session(shift_name: str = None, notes: str = None) -> int:
        ...
        try:
                ...
                for s in sessions:
                    cursor.execute(
                        ...
                        (
                            ...
                            s.product.id,
                            s.product.name,
                            s.product.large_unit,
                            s.product.conversion,
                            float(s.product.unit_price),
                            s.handover_qty,
                            s.closing_qty,
                            s.used_qty,
                            float(s.amount),
            ...
            raise DatabaseError(
                f"Failed to save session: {str(e)}", "save_current_session"
            )
    ...
    def get_by_id(history_id: int) -> Optional[SessionHistory]:
        ...
        try:
            with get_connection() as conn:
                ...
                cursor.execute(
                    "SELECT * FROM session_history WHERE id = ?", (history_id,)
                )
                row = cursor.fetchone()
                if not row:
                    return None
                ...
                history.items = [
                    SessionHistoryItem.from_row(r) for r in cursor.fetchall()
                ]

                return history
        except Exception as e:
            raise DatabaseError(
                f"Failed to get session history by ID: {str(e)}", "get_by_id"
            )
    ...
    def get_by_date_range(start_date: date, end_date: date) -> List[SessionHistory]:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to get session history by date range: {str(e)}",
                "get_by_date_range",
            )
    ...
    def delete(history_id: int) -> bool:
        ...
        try:
            with get_connection() as conn:
                ...
                cursor.execute(
                    "DELETE FROM session_history_items WHERE history_id = ?",
                    (history_id,),
                )
                cursor.execute(
...
class QuickPriceRepository:
    ...
    def add(name: str, price: float) -> int:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO quick_prices (name, price) VALUES (?, ?)", (name, price)
            )
    ...
    def update(id: int, name: str, price: float) -> bool:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE quick_prices SET name=?, price=? WHERE id=?", (name, price, id)
            )
            return cursor.rowcount > 0
...
class BankRepository:
    ...
    def get_all(limit: int = 100) -> List[BankNotification]:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM bank_history ORDER BY id DESC LIMIT ?", (limit,)
            )
