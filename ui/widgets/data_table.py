...
class DataTable(QTableWidget):
    ...
    def __init__(
        ...
        self._row_actions: List[tuple[str, str, Callable]] = (
            []
        )  # (name, icon, callback)
    ...
    def _setup_table(self, alternating_colors: bool, sortable: bool):
        ...
        self.cellDoubleClicked.connect(
            lambda row, col: self.row_double_clicked.emit(row)
        )
    ...
    def add_row_action(self, name: str, icon: str, callback: Callable):
        ...
        if "Actions" not in self.headers:
            ...
            self.setHorizontalHeaderItem(
                self.columnCount() - 1, QTableWidgetItem("Actions")
            )
            self.horizontalHeader().setSectionResizeMode(
                self.columnCount() - 1, QHeaderView.ResizeMode.Fixed
            )
    ...
    def _add_action_buttons(self, row_idx: int):
        ...
        for name, icon, callback in self._row_actions:
            ...
            btn.setToolTip(name)
            btn.clicked.connect(
                lambda checked, r=row_idx, n=name: self._on_action_clicked(n, r)
            )
