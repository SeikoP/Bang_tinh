...
class CalculatorToolView(QWidget):
    ...
    def _setup_ui(self):
        ...
        header_info.setStyleSheet(f"color: {AppColors.TEXT_SECONDARY}; font-weight: 700; font-size: 10px; letter-spacing: 1.5px;")
        ...
        self.prev_display.setStyleSheet(f"color: {AppColors.TEXT_SECONDARY}; font-size: 14px; min-height: 24px;")
        ...
        history_side.setStyleSheet(f"background-color: white; border-radius: 16px; border: 1px solid {AppColors.BORDER};")
        
        hist_layout = QVBoxLayout(history_side)
        hist_layout.setContentsMargins(15, 20, 15, 15)
        
        hist_label = QLabel("Dữ liệu Lịch sử")
        hist_label.setStyleSheet(f"color: {AppColors.TEXT}; font-weight: 800; font-size: 13px; margin-bottom: 8px;")
        ...
        clear_btn.setStyleSheet(f"QPushButton {{ background: transparent; color: #94a3b8; font-size: 11px; font-weight: 600; border: none; padding: 10px; }} QPushButton:hover {{ color: {AppColors.ERROR}; }}")
    ...
    def _on_btn(self, t):
        if t.isdigit():
            if self.new_num:
                ...
                if self.current_value == "0": self.current_value = t
                else: self.current_value += t
            ...
            if not self.new_num:
                ...
                if not self.current_value: self.current_value = "0"; self.new_num = True
                self._update_display()
                
        elif t == '±':
            if self.current_value != "0":
                if self.current_value.startswith('-'): self.current_value = self.current_value[1:]
                else: self.current_value = '-' + self.current_value
                self._update_display()
            ...
            try:
                ...
                self._update_display()
                self.new_num = True
            except: self.display.setText("Lỗi")
    ...
    def _calculate_final(self):
        if self.pending_op is None: return
    ...
    def _do_math(self, v1, v2, op):
        ...
        if op == '-': return v1 - v2
        if op == '*': return v1 * v2
        if op == '/': return v1 / v2 if v2 != 0 else 0
        return v2
    ...
    def _add_to_history(self, expr, res):
        frame = QFrame()
        frame.setStyleSheet(f"QFrame {{ background: #f8fafc; border-radius: 8px; padding: 8px; border: 1px solid #e2e8f0; }} QFrame:hover {{ border-color: {AppColors.PRIMARY}; }}")
        
        layout = QVBoxLayout(frame)
        layout.setContentsMargins(8, 8, 8, 8)
        
        eq = QLabel(expr)
        eq.setStyleSheet(f"color: #64748b; font-size: 11px;")
        eq.setWordWrap(True)
    ...
    def _on_special_action(self, action):
        try:
            ...
            if "VAT 10%" in action: res = curr * 1.1
            elif "VAT 8%" in action: res = curr * 1.08
            elif "Copy" in action:
