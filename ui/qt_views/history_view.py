...
class HistoryDetailDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        if self.history.items:
            ...
            table.setHorizontalHeaderLabels(["Sản phẩm", "Giao ca", "Chốt ca", "Đã dùng", "Thành tiền"])
            ...
            for row, item in enumerate(self.history.items):
                ...
                for col, val in enumerate([item.handover_qty, item.closing_qty, item.used_qty], 1):
...
class HistoryView(QWidget):
    ...
    def refresh_list(self):
        ...
        for row, h in enumerate(histories):
            ...
            self._set_cell(row, 3, f"{h.total_amount:,.0f}", center=True, fg=AppColors.SUCCESS, bold=True)
            
            notes_text = h.notes[:25] + "..." if h.notes and len(h.notes) > 25 else (h.notes or "—")
    ...
    def _delete_history(self, history_id):
        reply = QMessageBox.question(
            self, "Xác nhận", "Xóa phiên làm việc này?",
