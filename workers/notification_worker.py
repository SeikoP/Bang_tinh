...
class NotificationParser:
    ...
    PATTERNS = {
        'momo': {
            'source': 'MoMo',
            'amount': re.compile(r'[\+\-]?\s*(\d{1,3}(?:[,\.]\d{3})*(?:\.\d{2})?)\s*(?:VND|đ|d)?', re.IGNORECASE),
            'sender': re.compile(r'(?:từ|from|tu)\s+([A-Za-z0-9\s]+)', re.IGNORECASE),
        },
        'vietinbank': {
            'source': 'VietinBank',
            'amount': re.compile(r'[\+\-]?\s*(\d{1,3}(?:[,\.]\d{3})*(?:\.\d{2})?)\s*(?:VND|đ|d)?', re.IGNORECASE),
            'sender': re.compile(r'(?:TK|tai khoan|account)\s+([A-Za-z0-9\s]+)', re.IGNORECASE),
        },
        'vietcombank': {
            'source': 'Vietcombank',
            'amount': re.compile(r'[\+\-]?\s*(\d{1,3}(?:[,\.]\d{3})*(?:\.\d{2})?)\s*(?:VND|đ|d)?', re.IGNORECASE),
    ...
    def _extract_amount(cls, content: str, source: str) -> str:
        ...
        if bank_key in cls.PATTERNS:
            ...
            if match:
                ...
                if '+' in content or 'nhận' in content.lower() or 'nhan' in content.lower():
                    return f"+{amount}"
                elif '-' in content or 'chuyển' in content.lower() or 'chuyen' in content.lower():
                    return f"-{amount}"
                else:
                    return amount
