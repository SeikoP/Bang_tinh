...
def run_adb(args, timeout=5):
    """Run adb command"""
    try:
        result = subprocess.run([ADB_PATH] + args, 
...
def check_log_for_notification(timeout=3):
    ...
    success, stdout, _ = run_adb(['logcat', '-d', '-s', 'BankNotifier:D', 'HttpSender:D'])
...
def test_with_real_bank_package():
    ...
    success, stdout, _ = run_adb(['shell', 'dumpsys', 'activity', 'services', 'com.banknotifier'])
